...
export async function compressGeneratedResume(source: Buffer): Promise<Buffer> {
  const preparedResponse = await invoke("prepare", "compress_file_prepare", {
    contentType: "application/pdf",
    mediaKind: "generated-resume",
  });
  ...
  if (!preparedResponse.ok || !prepared.jobId || !prepared.upload?.url || !prepared.upload.fields) {
    throw new Error(prepared.detail ?? prepared.error ?? "Unable to prepare resume compression");
  }
  const form = new FormData();
  for (const [name, value] of Object.entries(prepared.upload.fields)) form.append(name, value);
  form.append("file", new Blob([source], { type: "application/pdf" }), "generated-resume.pdf");
  const staged = await fetch(prepared.upload.url, { method: "POST", body: form });
  if (!staged.ok) throw new Error("Unable to stage generated resume for compression");
  ...
  if (!compressed.ok) {
    ...
    throw new Error(failure?.detail ?? failure?.error ?? "Generated resume could not be compressed");
  }
